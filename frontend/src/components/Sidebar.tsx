...
export default function Sidebar({
  ...
  const loadSessions = useCallback(async () => {
    ...
    try { token = await getToken?.() ?? null } catch { /* anonymous */ }
    listSessions(token, anonId)
  ...
  const deleteSession = useCallback(async (e: React.MouseEvent, id: string) => {
    ...
    try {
      let token: string | null = null
      try { token = await getToken?.() ?? null } catch { /* anon */ }
  ...
  return (
    <aside
      className="fixed left-0 top-0 h-full bg-black text-neutral-200 flex flex-col z-30 select-none"
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-4 border-b border-white/10 shrink-0">
        {!isCollapsed && (
          ...
          >
            <div className="w-9 h-9 bg-white rounded-lg flex items-center justify-center text-black text-base font-black shrink-0">
              α
            </div>
            <span className="text-base font-bold text-white tracking-tight">AlphaLens</span>
          </button>
        )}
        <button
          onClick={onToggle}
          className={`p-1.5 rounded-lg text-neutral-400 hover:text-white hover:bg-white/5 transition-colors shrink-0 ${isCollapsed ? 'mx-auto' : ''}`}
      ...
      {/* New chat */}
      <div className="px-2 py-2.5 border-b border-white/10 shrink-0">
        <button
          onClick={onNewChat}
          className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-neutral-100 text-sm transition-colors ${
      ...
      {!isCollapsed ? (
        <div className="flex-1 overflow-y-auto py-2 px-2 space-y-0.5">
          {sessions.length === 0 && (
            <p className="text-xs text-neutral-500 px-3 py-2">No sessions yet</p>
          )}
          {sessions.map(s => (
            <div
              key={s.id}
            ...
            >
              ...
              <span className="truncate flex-1 text-left" title={s.title || 'Untitled'}>{s.title || 'Untitled'}</span>
              <button
